...
const AcademicInformation = ({ profileData, onSuccess, category = 'COLLEGE' }) => {
  ...
  // Debug logging for orientation fee
  console.log('🏫 Orientation Fee Debug:', {
    category: category,
    orientationNameId: formData.orientationNameId,
    orientationFee: orientationFee,
    orientationFeeLoading: orientationFeeLoading,
    orientationFeeError: orientationFeeError,
    shouldCallAPI: category?.toUpperCase() === 'SCHOOL' && formData.orientationNameId,
    orientationFeeKeys: orientationFee ? Object.keys(orientationFee) : 'No data',
    orientationFeeValues: orientationFee ? Object.values(orientationFee) : 'No data'
  });
  ...
  React.useEffect(() => {
    console.log('🔄 Orientation Fee useEffect triggered:', {
      category: category,
      categoryUpperCase: category?.toUpperCase(),
      isSchool: category?.toUpperCase() === 'SCHOOL',
      orientationFee: orientationFee,
    ...
    if (category?.toUpperCase() === 'SCHOOL' && orientationFee) {
      ...
      console.log('💰 Fee extraction attempt:', {
        'orientationFee': orientationFee,
        'orientationFee.data': orientationFee.data,
        'feeData': feeData,
        'feeData.fee': feeData.fee,
        'feeData.orientationFee': feeData.orientationFee,
        'feeData.orientation_fee': feeData.orientation_fee,
        'feeData.amount': feeData.amount,
        'feeData.price': feeData.price,
        'feeData.cost': feeData.cost,
        'finalFee': fee
      });
  ...
  const handleDropdownChange = (e) => {
    ...
    if (name === 'orientationName') {
      ...
      console.log('🎯 Orientation Selection Debug:', {
        selectedValue: value,
  ...
  const getFieldsByCategory = () => {
    ...
    if (category?.toUpperCase() === 'SCHOOL') {
      console.log('🏫 Filtering fields for SCHOOL category - hiding:', fieldsToHideForSchool);
      return allFields.filter(field => !fieldsToHideForSchool.includes(field.name));
    } else {
      console.log('🏫 Showing all fields for COLLEGE/DEGREE category');
      return allFields;
    }
  };
